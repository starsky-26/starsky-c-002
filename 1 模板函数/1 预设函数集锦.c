...
//13.低时间复杂度字符串匹配函数（kmp）及其辅助函数（getNext）：
...
//(1).整数域计算：
// 四则运算式→后缀表达式：
void infix_to_postfix(char *in, char *out) {// 待处理表达式在in中（以=结尾），后缀表达式输出到out中
	// 数据预处理（去=，长度检查）
	in[strlen(in)-1] = 0;
	if(strlen(in)>=1000) {// 如果字符串过长，需手动调整函数内数组大小
		printf("The string \"in\" is too long. Please modify the function \"infix_to_postfix\".\n");
		return;
	}
	
	// 数据转载并生成为intpoint型后缀表达式s1
	typedef struct {
		char sign;// 0=符号，1=数字
		char op;
		int x;
	...
	intpoint *s1 = malloc(sizeof(intpoint)*1002);
	
	
	// 数据转载为intpoint型&生成后缀表达式s1
	typedef struct {
		char sign;// 0=符号，1=数字
		char op;
		int x;
